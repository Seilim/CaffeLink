...
cflDebug::usage = "Prints internal parsed proto vars."
(* -------------------------------------------------------------------------- *)
...
initCaffeLinkModule[caffeProto_] := Module[{clProto, msgWithEnums, globalEnums},
	checkFile[caffeProto, "initCaffeLinkModule"];
...
until proper parameter name is found. Some timmes it is good idea to provide
...
checkFile[path_, fn_] := If[!FileExistsQ[path],
		Throw[StringJoin["In fn. '", fn, "' file not found: '", path, "'"]];
];
(* -------------------------------------------------------------------------- *)
...
trainNetSnapshotString[solver_, path_] := Module[{},
	checkFile[path, "trainNetSnapshotString"];
...
trainNetSnapshotFile[solverPath_, path_] := Module[{},
	checkFile[path, "trainNetSnapshotFile"];
	checkFile[solverPath, "trainNetSnapshotFile"];
...
trainNetWeightsString[solver_, path_] := Module[{},
	checkFile[path, "trainNetWeightsString"];
...
trainNetWeightsFile[solverPath_, path_] := Module[{},
	checkFile[path, "trainNetWeightsFile"];
	checkFile[solverPath, "trainNetWeightsFile"];
